...
main()
 ...
 2 解析启动参数（--profile 别名 forge-app-provided / --serve / --dev / --user-data-dir）
 3 bootstrap = await import('@lansi-ai/dsh-shell-runtime')   // 复用 dsh-app-boot 的 boot()
 4 ctx = boot('dsh-forge', desktopConfigPath, {
 ...
 7 Tray / 全局热键 / 协议注册（forge-* host 插件在 boot 中已就绪，此步只是窗口就绪后 attach）
 8 app.on('before-quit') → flush → 优雅关停 Host（sessions dispose → 持久化落盘）→ exit
 9 Host 异常退出（uncaughtException / crash）→ forge-host-restart 接管：记录 → relaunch
```
...
dsh-ui:///plugins/<id>/client.js?rev=  → 零端口 bundle 服务（forge-host-compat 从已装载插件包读取 bundle 产物）
dsh://open?session=…         → shell 注册 OS 层协议；解析后经 forge-host-protocol → 窗口聚焦/打开
```

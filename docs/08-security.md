...
- **桌面端动态 cordis 运行已启用**（2026-09-10 · 创造模式 · dogfood #23）：宿主半
  `cordis-host-runner` 在本进程的 `node:vm` 中求值模型生成的包代码，带浏览器半的包需经页面
  审批后才装载。风险口径同上一条——vm 隔离全局变量但**不是安全边界**（`ctx.fs`/`ctx.web`/`ctx.bash`
  等服务仍触达真实运行时），页面审批是「人在环」闸门；定义只存进程内存、不落盘，重启即清空。

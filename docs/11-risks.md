...
| R18 | 许可/商标边界（`@deepseek-ai` scope 使用） | 低×中 | 自有包用 `@dsh-desktop/*` scope；官方案例代码 MIT 引用注明 | closed（设计期内规避） |
...
| ID | 风险 | 评级 | 缓解 | 状态 |
| --- | --- | --- | --- | --- |
| R19 | 本地检出与 GitHub 最新 rc 差异导致文档假设过期 | 中×中 | ✅ 2026-08-25 已决策钉基线本地检出 `dsh-v0.1.0-rc.8`（事实表全部可复核）；rc.12 差异登记 sync-upstream 迁移表，M4 升级前核查 | closed |

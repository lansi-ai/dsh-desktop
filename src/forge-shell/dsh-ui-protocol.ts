...
import { generateFullBootScript, resolveBundleRequest, buildThirdPartyBundles } from '../forge-host/manifest.js'
import { dispatchHttpCompat, matchesCompatRoute } from '../forge-host/compat-webserver.js'
import { dispatchConnectionFetch } from '../forge-host/connection-fetch-bridge.js'
import { getActiveThemeId, resolveThemeDir, resolveGlobalIconPath } from '../forge-host/forge-theme.js'
import { log, logVerbose } from '../forge-host/log.js'

/**
 * dsh-ui:// 自定义协议：
 * 1. 优先加载官方 Web UI dist（@deepseek-ai/dsh-web-frontend/dist/）
 * 2. dist 不存在时回退到内置占位页面（src/forge-shell/web/index.html）
 *
...
function bootManifestScript(useDist: boolean): string {
  ...
  const boot = generateFullBootScript('forge-m1-ipc', extraBundles)

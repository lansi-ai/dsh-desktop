/**
 * @lansi-ai/dsh-forge-sidebar —— 侧栏壳插件（M6-P3 · D-20 全量自绘「先换壳不换内容」）。
 *
 ...
 *   - 品牌占位 fallback 待 M6-P2 `dsh-forge-brand` 替换。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 壳自有样式（布局容器背景/边框由 dsh-forge-layout 的 .dsh-forge-layout-sidebar 负责）。 */
    const CSS_TEXT = `
.dsh-forge-sidebar-root {
  display: flex;
...
.dsh-forge-sidebar-new-session:hover {
  background: light-dark(rgba(0, 0, 0, 0.05), rgba(255, 255, 255, 0.08));
}
.dsh-forge-sidebar-region {
  flex: 1;
  min-height: 0;
  overflow: hidden;
  display: flex;
...
.dsh-forge-sidebar-foot {
  padding: 6px 10px 10px;
  display: flex;
  flex-direction: column;
  gap: 4px;
}
.dsh-forge-sidebar-rail-label {
  font-size: 10px;
...
`
    ...
    function SidebarRoot({ collapsed, width, startSession, toggleSidebar, renderSlot }) {
      ...
      return h('div', {
        className: 'dsh-forge-sidebar-root',
        style: { width: collapsed ? lastWideWidth.current : width },
      },
        // 新会话按钮（宽列带文案，rail 仅图标）
        h('button', {
          type: 'button',
          className: 'dsh-forge-sidebar-new-session',
          title: '新建会话',
          'aria-label': '新建会话',
        ...
        h('div', { className: 'dsh-forge-sidebar-region' },
        ...
        // 同样同时承载这两者）
        h('div', { className: 'dsh-forge-sidebar-foot' },
          h('div', null, renderSlot('sidebar.settings', { wide })),
          h('div', { className: 'dsh-forge-sidebar-foot-action' },
    ...
    exports.apply = (ctx) => {
      ...
      document.getElementById('dsh-forge-sidebar-css')?.remove()
      const style = document.createElement('style')
      style.id = 'dsh-forge-sidebar-css'

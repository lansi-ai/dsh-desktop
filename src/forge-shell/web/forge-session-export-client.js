/**
 * @lansi-ai/dsh-forge-session-export —— Session 日志导出自有化（M6 外壳小件）。
 *
...
window.__ModuleLoader__.load({
  id: '@lansi-ai/dsh-forge-session-export',
  factory: (require) => {
    ...
    function getPrimitives() {
      ...
      try {
        ...
        console.warn('[dsh-forge-session-export] 官方 primitives 不可用，回退内置弹层:', error)
      }
    ...
    function injectStyles() {
      const tagId = '@lansi-ai/dsh-forge-session-export/HeaderAction.module.css'
      if (document.querySelector(`style[data-plugin-css="${tagId}"]`) !== null) return
      const tag = document.createElement('style')
      tag.dataset.plugin = '@lansi-ai/dsh-forge-session-export'
...
.dsh-forge-session-export-button:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}
.dsh-forge-session-export-button:disabled{color:var(--dsw-alias-label-dimmed);cursor:wait}
.dsh-forge-session-export-button span,.dsh-forge-session-export-button svg{flex:none}
.dsh-forge-session-export-button span{white-space:nowrap}`
    ...
    function SessionExportHeaderAction(props) {
      ...
      return h(React.Fragment, null,
        h('button', {
          type: 'button',
          className: 'dsh-forge-session-export-button',
          disabled: busy === true,
          'aria-busy': busy === true,
    ...
    exports.apply = (ctx) => {
      ...
      ctx.effect(() => async () => {
        await dispose()
      }, 'forge-session-export: browser download lifecycle')
      ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'forge-session-export: dictionaries')

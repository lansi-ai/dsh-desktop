/**
 * @lansi-ai/dsh-forge-ui-icons —— 官方 UI 内部图标主题化覆盖层。
 *
 ...
 *   替换为主题图标。替换呈现两条路径：
 *     - themeIcon 服务（@lansi-ai/dsh-forge-icons）可用 → **内联上色**：单色稿
 ...
 *   改变 path 特征，需重新登记；特征真源 = forge-theme.ts 槽位注册表 match 字段）。
 *
...
window.__ModuleLoader__.load({
  id: '@lansi-ai/dsh-forge-ui-icons',
  factory: () => {
    ...
    async function install() {
      try {
        ...
        if (!wasActive) {
          ...
          wasActive = true
          console.log(`[dsh-forge-ui-icons] 图标覆盖层已激活（${rules.length} 条映射，themeIcon=${themeIcon !== null ? '内联' : 'img 兜底'}）`)
        }
        refreshReplaced()
    ...
    function uninstall() {
      ...
      console.log('[dsh-forge-ui-icons] 当前主题无映射规则，覆盖层已停用')
    }

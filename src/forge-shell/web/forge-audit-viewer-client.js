/**
 * @lansi-ai/dsh-forge-audit-viewer —— 审计查看器 renderer 注入（M3·b2 审计查询）。
 *
 ...
 *   - 下行：onDesktopEvent（通过 forge-settings:open 等事件触发打开）
 *
 * 注：本文件为浏览器侧 bundle（含 window 全局），不参与 Node 编译。
 */
window.__ModuleLoader__.load({
  id: '@lansi-ai/dsh-forge-audit-viewer',
  factory: (require) => {
    ...
    function installDesktopEventListener() {
      ...
      return bridge.onDesktopEvent((event) => {
        ...
        if (action === 'forge-audit:open') {
          openViewer()
        } else if (action === 'forge-audit:close') {
          closeViewer()
        }
      })
    }

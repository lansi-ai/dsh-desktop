/**
 * dsh-forge `ctx.desktop` 聚合服务（M2·地基 forge-host-core / forge-api）。
 *
...
export async function installDesktopCore(ctx: unknown, options?: InstallDesktopCoreOptions): Promise<void> {
    ...
    private writeAuditRecord(record: Record<string, unknown>): void {
      this.writeQueue = this.writeQueue.then(async () => {
        try {
          ...
          log.warn('[dsh-forge] 审计日志写入失败:', err)
        }
      })
    }
    ...
    writeConfig<T>(key: string, value: T): void {
      ...
      if (scope !== null) {
        void scope.update({ [key]: value }).catch(() => {
          log.warn('[dsh-forge] desktop 配置持久化失败:', key)
        })
      }
    }
  ...
  log.ok('[dsh-forge] ctx.desktop 聚合服务已注入（onAction/log/config/event）')
}

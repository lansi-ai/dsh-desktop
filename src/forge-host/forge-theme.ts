/**
 * dsh-forge 桌面图标主题服务（图标主题 / 颜色主题拆分 · 图标侧）。
 *
 * 图标分两类归属，互不牵连：
 ...
 * @lansi-ai/dsh-forge-icons）；`format: 'png'` 原色呈现。
 */
const RAW_ICON_SLOTS: readonly Omit<IconSlot, 'scope'>[] = [
  {
    ...
    label: '应用图标（浅色底）',
    group: '应用图标',
    plugin: 'forge-host（main.ts / forge-tray.ts）',
  ...
  },
  {
    ...
    label: '托盘图标（浅色底）',
    group: '托盘图标',
    plugin: 'forge-host（forge-tray.ts）',
  ...
  },
  {
    ...
    label: '标题栏品牌标记（浅色）',
    group: '品牌标记',
    plugin: '@lansi-ai/dsh-forge-titlebar（标题栏左上品牌 logo）',
  ...
  },
  {
    ...
    label: '窗控「最小化」图标',
    group: '标题栏',
    plugin: '@lansi-ai/dsh-forge-titlebar',
  ...
  },
  {
    ...
    label: '侧栏「设置」入口图标',
    group: '设置面板',
    plugin: '@lansi-ai/dsh-forge-settings-shell',
  ...
  ...([
    ...
    ...slot,
    group: '工作区侧栏',
    plugin: '官方 ui-workspace（@lansi-ai/dsh-forge-ui-icons 覆盖层）',
    format: 'svg' as const,
    size: 16,
...
// ── 模块级激活状态（main.ts / forge-tray.ts / dsh-ui-protocol.ts 同步读取）────
...
/** 主题资源根目录（__dirname = dist/forge-host → dist/resources/themes）。 */
...
function resolveDefaultIconPath(kind: ThemeIconKind, dark: boolean): string {
  return join(__dirname, '..', 'forge-shell', 'web', dark ? ICON_FILES[kind].dark : ICON_FILES[kind].light)
}
...
/**
 * 解析当前应用/托盘图标绝对路径（同步；main.ts loadAppIcon / forge-tray
 ...
 * 真源 = 内置 web 目录（`dist/forge-shell/web/`）而非图标包：品牌是全局身份标识，
...
/** 官方 UI 覆盖规则（icons/ui-overrides.json 条目，@lansi-ai/dsh-forge-ui-icons 消费）。 */
...
export function registerDesktopThemeMethods(desktop: DesktopCore): void {
  ...
  const handleUpload = async (params: unknown): Promise<IconThemeUploadResult> => {
    ...
    // 覆盖层（@lansi-ai/dsh-forge-ui-icons）随后按新规则做 DOM 替换

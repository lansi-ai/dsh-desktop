/**
 * 自有化凭据 provider（@lansi-ai/dsh-forge-credentials · M4 数据目录选择配套）。
 *
 * 完全重写官方 `@deepseek-ai/dsh-credentials-local` 的 provider 行为（M6 自有化模式）：
 *   - 文档层：forge-credentials-document.ts（version-1 格式与官方 CLI 互换兼容）；
...
import { assertOwnerOnly, isENOENT, createCredentialsDocument } from './forge-credentials-document.js'
import { canonicalizeWatchPath, resolveDshHome } from './forge-home-paths.js'
...
export async function installDesktopCredentials(hostCtx: unknown, options: DesktopCredentialsOptions = {}): Promise<void> {
  ...
  // 路径解析用自有实现（forge-home-paths.ts，语义对齐官方 dsh-home-paths），
  ...
  class DesktopCredentialProvider extends CredentialProvider {
    ...
    async *[Service.init](): AsyncGenerator<() => Promise<void>> {
      ...
      watcher.on('error', (error: unknown) => {
        ctx.logger.warn('forge-credentials: watcher error on %s', spec.filename)
    ...
    override async set(ref: CredentialRef, value: string): Promise<void> {
      if (value.length === 0) {
        throw new Error(`forge-credentials: an empty value cannot be stored for "${ref}"; use unset`)
      }
    ...
    override async modifyRecord(
      ...
      if (this.closed) throw new Error(`forge-credentials is disposed: cannot modify "${key}"`)
      return this.enqueue(async () => {
        if (this.closed) throw new Error(`forge-credentials was disposed before the queued "${key}" modify ran`)
    ...
    override async deleteRecord(key: CredentialKey): Promise<void> {
      if (this.closed) throw new Error(`forge-credentials is disposed: cannot delete "${key}"`)
      await this.enqueue(async () => {
        if (this.closed) throw new Error(`forge-credentials was disposed before the queued "${key}" delete ran`)
        await mkdir(dirname(spec.filename), { recursive: true, mode: 0o700 })
        await withFileLock(
          spec.filename,
          async () => {
    ...
    private queueRefresh(): void {
      this.enqueue(() => this.refresh()).catch((error: unknown) => {
        ctx.logger.error('forge-credentials: reload commit failed at %s', spec.filename)
    ...
    private async write(ref: CredentialRef, value: string | undefined): Promise<void> {
      ...
      if (this.closed) throw new Error(`forge-credentials is disposed: cannot ${verb} "${ref}"`)
      this.assertUnshadowed(ref, verb)
      return this.enqueue(async () => {
        if (this.closed) throw new Error(`forge-credentials was disposed before the queued "${ref}" ${verb} ran`)
        this.assertUnshadowed(ref, verb)
        await mkdir(dirname(spec.filename), { recursive: true, mode: 0o700 })
        await withFileLock(
          spec.filename,
    ...
    private assertUnshadowed(ref: CredentialRef, verb: string): void {
      if (this.inherited(ref) !== undefined) {
        throw new Error(
          `forge-credentials: "${ref}" is supplied read-only by the launching environment, ` +
      ...
      if (doc.renderFlatLayoutMigration(text) !== undefined) {
        await withFileLock(
          ...
          async () => {
            ...
            ctx.logger.info('forge-credentials: migrated %s to the version-1 layout; values are unchanged', spec.filename)
            text = migrated
          },
          { waitMs: DOCUMENT_LOCK_WAIT_MS },
        )
    ...
    private async refresh(): Promise<void> {
      ...
      try {
        ...
        ctx.logger.warn('forge-credentials: reload failed at %s; keeping the last good document', spec.filename)
        ctx.logger.warn(error)
      }
    }
  ...
  log.ok(`[dsh-boot] forge-credentials 已注入（${filename}${spec.watch ? '，watch 开启' : ''}）`)
}

/**
 * 自有化凭据文档层（@lansi-ai/dsh-forge-credentials · M4 数据目录选择配套）。
 *
...
export async function assertOwnerOnly(filename: string): Promise<void> {
  ...
  throw new Error(
    `forge-credentials: ${filename} is readable beyond its owner (mode ${(mode & 0o777).toString(8)}); ` +
...
function asSection(section: unknown, name: string, filename: string): Record<string, unknown> {
  ...
  if (typeof section !== 'object' || Array.isArray(section)) {
    throw new TypeError(`forge-credentials: "${name}" in ${filename} must be a mapping`)
  }
...
function parseRecord(deps: CredentialDocDeps, key: string, value: unknown, filename: string): CredentialRecord {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new TypeError(`forge-credentials: record "${key}" in ${filename} must be a mapping`)
  }
  ...
  const assertFields = (allowed: string[]): void => {
    for (const field of Object.keys(fields)) {
      if (!allowed.includes(field)) {
        throw new Error(`forge-credentials: record "${key}" in ${filename} has unknown field "${field}"`)
      }
    }
  }
  if (kind === 'api-key') {
    ...
    if (apiKey !== undefined && (typeof apiKey !== 'string' || apiKey.length === 0)) {
      throw new TypeError(`forge-credentials: record "${key}" in ${filename} has a non-string or empty key`)
    }
    let env: Record<string, string> | undefined
    if (fields['env'] !== undefined) {
      if (typeof fields['env'] !== 'object' || fields['env'] === null || Array.isArray(fields['env'])) {
        throw new TypeError(`forge-credentials: record "${key}" in ${filename} has a non-mapping env`)
      }
      env = {}
      for (const [name, entryValue] of Object.entries(fields['env'] as Record<string, unknown>)) {
        ...
        if (typeof entryValue !== 'string' || entryValue.length === 0) {
          throw new TypeError(`forge-credentials: record "${key}" env "${name}" in ${filename} must be a non-empty string`)
        }
        env[name] = entryValue
      }
    }
  ...
  if (kind === 'grant') {
    ...
    if (!('payload' in fields)) {
      throw new Error(`forge-credentials: record "${key}" in ${filename} has no payload`)
    }
    assertJsonValue(`record "${key}" payload in ${filename}`, fields['payload'], new Set())
    return { kind: 'grant', payload: fields['payload'] }
  }
  if (kind === undefined) throw new Error(`forge-credentials: record "${key}" in ${filename} has no kind`)
  throw new Error(`forge-credentials: record "${key}" in ${filename} has unknown kind ${JSON.stringify(kind)}`)
}
...
function assertJsonValue(where: string, value: unknown, seen: Set<object>): void {
  ...
  if (typeof value === 'number') {
    ...
    throw new TypeError(`forge-credentials: ${where} holds a non-finite number`)
  }
  if (typeof value === 'object') {
    if (seen.has(value as object)) throw new TypeError(`forge-credentials: ${where} is cyclic`)
  ...
  throw new TypeError(`forge-credentials: ${where} holds a value JSON cannot represent`)
}
...
function assertStorableApiKey(deps: CredentialDocDeps, key: string, record: CredentialRecord): void {
  ...
  if (record.key !== undefined && record.key.length === 0) {
    throw new TypeError(`forge-credentials: record "${key}" has an empty key; omit the field instead`)
  }
  for (const [name, value] of Object.entries(record.env ?? {})) {
    deps.credentialRef(name)
    if (value.length === 0) {
      throw new TypeError(`forge-credentials: record "${key}" env "${name}" must be a non-empty string`)
    }
  }
}
...
export function createCredentialsDocument(deps: CredentialDocDeps) {
  ...
  function parseCredentialsDocument(text: string, filename: string): ParsedCredentialsDocument {
    ...
    if (document.errors.length > 0) {
      throw new Error(
        `forge-credentials: invalid document at ${filename}: ${document.errors.map(describeYamlError).join('; ')}`,
      )
    }
    const root = document.toJS() ?? {}
    if (typeof root !== 'object' || root === null || Array.isArray(root)) {
      throw new TypeError(`forge-credentials: ${filename} must be a mapping`)
    }
    ...
    if (!('version' in fields)) {
      throw new Error(
        `forge-credentials: ${filename} uses the pre-release flat layout. Add \`version: 1\` and nest the ` +
        `existing ${keys.length} ${keys.length === 1 ? 'entry' : 'entries'} under \`refs:\`. No values need to change.`,
      )
    }
    if (fields['version'] !== DOCUMENT_VERSION) {
      throw new Error(
        `forge-credentials: ${filename} declares version ${JSON.stringify(fields['version'])}; ` +
        `this build reads version ${DOCUMENT_VERSION}`,
      )
    }
    for (const key of keys) {
      if (key !== 'version' && key !== 'refs' && key !== 'records') {
        throw new Error(`forge-credentials: unknown top-level key "${key}" in ${filename}`)
      }
    }
    ...
    for (const [key, value] of Object.entries(asSection(fields['refs'], 'refs', filename))) {
      ...
      if (typeof value !== 'string') {
        throw new TypeError(`forge-credentials: the value for "${key}" in ${filename} must be a string`)
      }
      if (value.length === 0) {
        throw new Error(`forge-credentials: the value for "${key}" in ${filename} is empty; remove the key instead`)
      }
      refs.set(key, value)
    }

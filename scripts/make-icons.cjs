// 托盘图标生成：把官方 harness favicon.svg 光栅化为黑白双版 PNG（tray 64）。
...
// 直接字符串替换 fill 生成白色变体）。
// 产物：src/desktop-shell/web/tray-icon-{light,dark}.png
//   - light 版 = 黑色 logo（浅色托盘用）
//   - dark  版 = 白色 logo（深色托盘用）
// 运行期由 desktop-tray.ts 按 nativeTheme.shouldUseDarkColors 动态选用。
// 注：应用图标（app-icon-{light,dark}.png）自 2026-09-09 起改为自有品牌 logo，
//     由 scripts/process-logo.cjs 从项目根 logo.png 生成，本脚本不再产出。
